...
public class WebSubsystemElement extends AbstractSubsystemElement<WebSubsystemElement> {
    ...
    public long elementHash() {
        ...
        if (config != null)
            cksum = Long.rotateLeft(cksum, 1) ^ config.elementHash();
        return cksum;
    }
    ...
    public void activate(ServiceActivatorContext context) {
        // Add the web server
        final BatchServiceBuilder<WebServer> service = context.getBatchBuilder().addService(JBOSS_WEB_SERVER, new WebServerService(this.config, this.virtuals.values()));
